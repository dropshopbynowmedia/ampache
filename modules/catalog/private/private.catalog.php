...
/**
 * Private Catalog Class
 *
 * This class handles all actual work in regards to private catalogs.
 *
 */
class Catalog_private extends Catalog
{
    private $version        = '000001';
    private $type           = 'private';
    private $description    = 'Private Catalog';
    ...
    /**
     * get_owner
     * This returns the current catalog owner
     */
    ...
    /**
     * is_installed
     * This returns true or false if private catalog is installed
     */
    public function is_installed()
    {
        $sql        = "SHOW TABLES LIKE 'catalog_private'";
    ...
    /**
     * install
     * This function installs the private catalog
     */
    public function install()
    {
        $sql = "CREATE TABLE `catalog_private` (`id` INT( 11 ) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY , " .
    ...
    public static function get_from_path($path)
    {
        ...
        $sql        = "SELECT `catalog_id`, `path` FROM `catalog_private`";
    ...
    public static function create_type($catalog_id, $data)
    {
        ...
        if (!Core::is_readable($path)) {
            debug_event('private.catalog', 'Cannot add catalog at unopenable path ' . $path, 1);
        ...
        $sql        = 'SELECT `id` FROM `catalog_private` WHERE `path` = ?';
        $db_results = Dba::read($sql, array($path));

        if (Dba::num_rows($db_results)) {
            debug_event('private.catalog', 'Cannot add catalog with duplicate path ' . $path, 1);
            /* HINT: directory (file path) */
            AmpError::add('general', sprintf(T_('This path belongs to an existing private Catalog: %s'), $path));

            return false;
        }

        $sql = 'INSERT INTO `catalog_private` (`path`, `catalog_id`) VALUES (?, ?)';
    ...
    public function add_files($path, $options)
    {
        ...
        debug_event('private.catalog', UI::format_bytes(memory_get_usage(true)), 5);
        ...
        if (!is_resource($handle)) {
            debug_event('private.catalog', "Unable to open $path", 3);
            /* HINT: directory (file path) */
        ...
        if (!chdir($path)) {
            debug_event('private.catalog', "Unable to chdir to $path", 2);
            /* HINT: directory (file path) */
        ...
        while (false !== ($file = readdir($handle))) {
            ...
            debug_event('private.catalog', "Reading $file inside $path", 5);
            debug_event('private.catalog', "Memory usage: " . (string) UI::format_bytes(memory_get_usage(true)), 5);
        ...
        debug_event('private.catalog', "Finished reading $path , closing handle", 5);
    ...
    public function add_file($full_file, $options)
    {
        ...
        if (AmpConfig::get('no_symlinks')) {
            if (is_link($full_file)) {
                debug_event('private.catalog', "Skipping symbolic link $full_file", 5);
        ...
        if (is_dir($full_file)) {
            ...
            /* Change the dir so is_dir works correctly */
            if (!chdir($full_file)) {
                debug_event('private.catalog', "Unable to chdir to $full_file", 2);
                /* HINT: directory (file path) */
        ...
        if ($is_audio_file || $is_video_file || $is_playlist) {
            ...
            if (!$file_size) {
                debug_event('private.catalog', "Unable to get filesize for $full_file", 2);
                /* HINT: FullFile */
            ...
            if (!Core::is_readable($full_file)) {
                ...
                debug_event('private.catalog', "$full_file is not readable by Ampache", 2);
            ...
            if (function_exists('iconv')) {
                ...
                if (!$convok) {
                    debug_event('private.catalog', $full_file . ' has non-' . $site_charset . ' characters and can not be indexed, converted filename:' . $enc_full_file, 1);
                    /* HINT: FullFile */
            ...
            if ($is_playlist) {
                debug_event('private.catalog', 'Found playlist file to import: ' . $full_file, 5);
                ...
                if (count($this->get_gather_types('music')) > 0) {
                    if ($is_audio_file) {
                        $this->insert_private_song($full_file, $options);
                    } else {
                        debug_event('private.catalog', $full_file . " ignored, bad media type for this music catalog.", 5);
                    ...
                    if (count($this->get_gather_types('video')) > 0) {
                        if ($is_video_file) {
                            $this->insert_private_video($full_file, $options);
                        } else {
                            debug_event('private.catalog', $full_file . " ignored, bad media type for this video catalog.", 5);
            ...
            debug_event('private.catalog', "$full_file ignored, non-audio file or 0 bytes", 5);
        ...
        if ($this->gather_types == "podcast") {
            ...
            if ($options['parse_playlist'] && count($this->_playlists)) {
                ...
                foreach ($this->_playlists as $full_file) {
                    debug_event('private.catalog', 'Processing playlist: ' . $full_file, 5);
    ...
    {
        debug_event('private.catalog', 'Verify starting on ' . $this->name, 5);
        set_time_limit(0);
        ...
        debug_event('private.catalog', "Verify finished, $total_updated updated in " . $this->name, 5);
    ...
    {
        debug_event('private.catalog', "Verify starting chunk $chunk", 5);
        ...
        while ($row = Dba::fetch_assoc($db_results)) {
            ...
            if (!Core::is_readable(Core::conv_lc_file($row['file']))) {
                ...
                debug_event('private.catalog', $row['file'] . ' does not exist or is not readable', 5);
                continue;
            }
     ...
     *
     * Removes private songs that no longer exist.
     */
    public function clean_catalog_proc()
    {
        if (!Core::is_readable($this->path)) {
            ...
            // catalog root.
            debug_event('private.catalog', 'Catalog path:' . $this->path . ' unreadable, clean failed', 1);
        ...
        foreach (array('video', 'song') as $media_type) {
            ...
            if (!file_exists($this->path)) {
                if ($dead_count >= $total) {
                    debug_event('private.catalog', 'All files would be removed. Doing nothing.', 1);
    ...
    {
        debug_event('private.catalog', "Starting chunk $chunk", 5);
        $dead  = array();
        $count = $chunk * $chunk_size;
        ...
        while ($results = Dba::fetch_assoc($db_results)) {
            debug_event('private.catalog', 'Cleaning check on ' . $results['file'] . '(' . $results['id'] . ')', 5);
            $count++;
            if (UI::check_ticker()) {
            ...
            if (!file_exists(Core::conv_lc_file($results['file'])) || $file_info < 1) {
                debug_event('private.catalog', 'File not found or empty: ' . $results['file'], 5);
                /* HINT: filename (file path) */
                ...
                if (!Core::is_readable(Core::conv_lc_file($results['file']))) {
                    debug_event('private.catalog', $results['file'] . ' is not readable, but does exist', 1);
                }
            }
        }
    ...
    public function clean_file($file, $media_type = 'song')
    {
        ...
        if (!file_exists(Core::conv_lc_file($file)) || $file_info < 1) {
            debug_event('private.catalog', 'File not found or empty: ' . $file, 5);
            /* HINT: filename (file path) */
            ...
            if (!Core::is_readable(Core::conv_lc_file($file))) {
                debug_event('private.catalog', $file . ' is not readable, but does exist', 1);
            }
        }
    } //clean_file
     ...
     * @throws Exception
     */
    private function insert_private_song($file, $options = array())
    {
        ...
        if (count($this->get_gather_types('music')) > 0) {
            if (AmpConfig::get('catalog_check_duplicate')) {
                if (Song::find($results)) {
                    debug_event('private.catalog', 'skipping_duplicate ' . $file, 5);
            ...
            if ($options['move_match_pattern']) {
                ...
                if ($patres['artist'] != $results['artist'] || $patres['album'] != $results['album'] || $patres['track'] != $results['track'] || $patres['title'] != $results['title']) {
                    ...
                    if (count($filepart) > $cntslash) {
                        ...
                        debug_event('private.catalog', 'Unmatching pattern, moving `' . $file . '` to `' . $mvfile . '`...', 5);
                        ...
                        if (rename($file, $mvfile)) {
                            ...
                            debug_event('private.catalog', 'File rename failed', 3);
                        }
                    }
                }
            }
        ...
        if ($id) {
            ...
            if (array_key_exists('rating', $results) && is_array($results['rating'])) {
                ...
                foreach ($results['rating'] as $user => $rating) {
                    debug_event('private.catalog', "Setting rating for Song $id to $rating for user $user", 5);
     ...
     * @throws Exception
     */
    public function insert_private_video($file, $options = array())
    {
     ...
     * @return boolean
     */
    public function check_private_mp3($full_file, $gather_type = '')
    {
        $file_date = filemtime($full_file);
        if ($file_date < $this->last_add) {
            debug_event('private.catalog', 'Skipping ' . $full_file . ' File modify time before last add run', 3);
